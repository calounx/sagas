...
function get_entity_preview_attributes( int $entity_id, string $entity_type ): array {
	...
	$priority_attrs = array(
		'character' => array( 'species', 'affiliation', 'homeworld', 'birth_date' ),
		'location'  => array( 'type', 'planet', 'region', 'population' ),
		'event'     => array( 'date', 'location', 'participants', 'outcome' ),
		'faction'   => array( 'type', 'leader', 'founded', 'allegiance' ),
		'artifact'  => array( 'type', 'creator', 'power', 'location' ),
		'concept'   => array( 'category', 'origin', 'significance' ),
	);

	$priority = $priority_attrs[ $entity_type ] ?? array();
