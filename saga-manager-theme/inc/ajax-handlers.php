...
function saga_ajax_track_view(): void {
    ...
    if ($success) {
        ...
        wp_send_json_error(['message' => 'View not tracked'], 400);
    }
}
...
function saga_ajax_track_duration(): void {
    ...
    if ($success) {
        ...
        wp_send_json_error(['message' => 'Duration not tracked'], 400);
    }
}
...
function saga_ajax_track_custom_action(): void {
    ...
    switch ($action_name) {
        case 'bookmark_added':
            ...
            wp_send_json_success(['message' => 'Bookmark tracked']);
            break;

        case 'bookmark_removed':
            Saga_Analytics::update_bookmark_count($entity_id, -1);
            wp_send_json_success(['message' => 'Bookmark removal tracked']);
            break;

        case 'annotation_added':
            Saga_Analytics::update_annotation_count($entity_id, 1);
            wp_send_json_success(['message' => 'Annotation tracked']);
            break;

        case 'annotation_removed':
            Saga_Analytics::update_annotation_count($entity_id, -1);
            wp_send_json_success(['message' => 'Annotation removal tracked']);
            break;
...
function saga_ajax_get_popularity_stats(): void {
    ...
    if (!$entity_id) {
        wp_send_json_error(['message' => 'Missing entity_id'], 400);
    }
...
function saga_ajax_get_trending(): void {
    ...
    if (!in_array($period, ['hourly', 'daily', 'weekly'], true)) {
        $period = 'weekly';
    }
...
function saga_enqueue_analytics_scripts(): void {
    ...
    wp_localize_script('saga-view-tracker', 'sagaAnalytics', [
