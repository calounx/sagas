...
class SagaAjaxHandler {
	...
	public function registerEndpoints(): void {
		...
		add_action( 'wp_ajax_saga_filter_entities', array( $this, 'filterEntities' ) );
		add_action( 'wp_ajax_nopriv_saga_filter_entities', array( $this, 'filterEntities' ) );

		// Search entities (public and authenticated)
		add_action( 'wp_ajax_saga_search_entities', array( $this, 'searchEntities' ) );
		add_action( 'wp_ajax_nopriv_saga_search_entities', array( $this, 'searchEntities' ) );

		// Autocomplete search (public and authenticated)
		add_action( 'wp_ajax_saga_autocomplete_search', array( $this, 'autocompleteSearch' ) );
		add_action( 'wp_ajax_nopriv_saga_autocomplete_search', array( $this, 'autocompleteSearch' ) );

		// Get relationships (public and authenticated)
		add_action( 'wp_ajax_saga_get_relationships', array( $this, 'getRelationships' ) );
		add_action( 'wp_ajax_nopriv_saga_get_relationships', array( $this, 'getRelationships' ) );
	}
	...
	public function filterEntities(): void {
		...
		$validTypes = array( 'character', 'location', 'event', 'faction', 'artifact', 'concept' );
		...
		try {
			...
			$this->sendSuccess(
				array(
					'results' => $allResults,
					'grouped' => array_map(
						function ( $items ) use ( $query ) {
							return array_map(
								function ( $entity ) use ( $query ) {
	...
	public function getRelationships(): void {
		...
		// Validate direction
		$validDirections = array( 'outgoing', 'incoming', 'both' );
