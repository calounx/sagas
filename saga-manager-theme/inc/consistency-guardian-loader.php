...
function saga_consistency_enqueue_editor_assets(): void
{
    ...
    if (!$screen || !in_array($screen->post_type, ['saga_entity'], true)) {
        return;
    }
    ...
    // Classic Editor
        ...
        wp_enqueue_script(
            ...
            ['jquery', 'saga-consistency-realtime'],
            $version,
            true
        );
...
{
    register_rest_route('saga/v1', '/consistency/scan', [
        'methods' => 'POST',
    ...
    register_rest_route('saga/v1', '/consistency/issues', [
        'methods' => 'GET',
    ...
    register_rest_route('saga/v1', '/consistency/issues/(?P<id>\d+)', [
        'methods' => 'GET',
...
function saga_consistency_rest_scan(WP_REST_Request $request): WP_REST_Response
{
    ...
    if ($sagaId === 0) {
        return new WP_REST_Response(['error' => 'Invalid saga ID'], 400);
    }
