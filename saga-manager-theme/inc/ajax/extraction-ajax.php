 ...
 */
add_action('wp_ajax_saga_start_extraction', function() {
    // Security checks
    ...
    $rate_key = "saga_extraction_rate_{$user_id}";
    $recent_jobs = get_transient($rate_key) ?: 0;

    if ($recent_jobs >= 10) {
        wp_send_json_error([
            'message' => 'Rate limit exceeded. Maximum 10 extractions per hour.',
        ], 429);
    }
    ...
    // Validation
    if ($saga_id === 0) {
        wp_send_json_error(['message' => 'Invalid saga ID'], 400);
    }

    if (empty(trim($source_text))) {
        wp_send_json_error(['message' => 'Source text cannot be empty'], 400);
    }

    if (mb_strlen($source_text) > 100000) {
        wp_send_json_error(['message' => 'Text too long. Maximum 100,000 characters.'], 400);
    }

    if (!in_array($chunk_size, [1000, 2500, 5000, 10000], true)) {
        wp_send_json_error(['message' => 'Invalid chunk size'], 400);
    }
    ...
    try {
        ...
        set_transient($rate_key, $recent_jobs + 1, HOUR_IN_SECONDS);
 ...
 */
add_action('wp_ajax_saga_get_extraction_progress', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
 ...
 */
add_action('wp_ajax_saga_load_extracted_entities', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
    ...
    try {
        ...
        // Apply filters
        $filtered_entities = array_filter($all_entities, function($entity) use ($filter_type, $filter_status, $filter_confidence) {
        ...
        $formatted_entities = array_map(function($entity) use ($repository) {
 ...
 */
add_action('wp_ajax_saga_approve_entity', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
    ...
    try {
        ...
        if (!$entity) {
            wp_send_json_error(['message' => 'Entity not found'], 404);
        }
 ...
 */
add_action('wp_ajax_saga_reject_entity', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
 ...
 */
add_action('wp_ajax_saga_bulk_approve_entities', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
    ...
    $entity_ids = isset($_POST['entity_ids']) ? array_map('absint', (array)$_POST['entity_ids']) : [];

    if (empty($entity_ids)) {
        wp_send_json_error(['message' => 'No entities provided'], 400);
    }
 ...
 */
add_action('wp_ajax_saga_batch_create_approved', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
 ...
 */
add_action('wp_ajax_saga_resolve_duplicate', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
    ...
    if ($duplicate_id === 0) {
        wp_send_json_error(['message' => 'Invalid duplicate ID'], 400);
    }

    if (!in_array($action, ['confirmed_duplicate', 'marked_unique'], true)) {
 ...
 */
add_action('wp_ajax_saga_load_job_history', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
 ...
 */
add_action('wp_ajax_saga_cancel_extraction_job', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
 ...
 */
add_action('wp_ajax_saga_get_extraction_stats', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
 ...
 */
add_action('wp_ajax_saga_estimate_extraction_cost', function() {
    check_ajax_referer('saga_extraction_nonce', 'nonce');
