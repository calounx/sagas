...
class TimelineDataHandler {
    ...
    public static function register(): void {
        ...
        add_action('wp_ajax_get_timeline_events', [self::class, 'getTimelineEvents']);
        add_action('wp_ajax_nopriv_get_timeline_events', [self::class, 'getTimelineEvents']);

        add_action('wp_ajax_get_timeline_event_details', [self::class, 'getEventDetails']);
        add_action('wp_ajax_nopriv_get_timeline_event_details', [self::class, 'getEventDetails']);
    }
    ...
    public static function getTimelineEvents(): void {
        ...
        if (!$saga_id) {
            wp_send_json_error(['message' => 'Saga ID is required'], 400);
            return;
        }
        ...
        if (!$saga) {
            wp_send_json_error(['message' => 'Saga not found'], 404);
            return;
        }
        ...
        // Process events
        $processed_events = array_map(function($event) use ($wpdb) {
            // Decode JSON fields
    ...
    public static function getEventDetails(): void {
        ...
        if (!$event_id) {
            wp_send_json_error(['message' => 'Event ID is required'], 400);
            return;
        }
    ...
    private static function getRelatedEvents(int $entity_id, $wpdb): array {
        ...
        $related_entity_ids = $wpdb->get_col($wpdb->prepare(
        ...
        $related_event_ids = $wpdb->get_col($wpdb->prepare(
