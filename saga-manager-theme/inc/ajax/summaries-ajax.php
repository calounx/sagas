 ...
 */
add_action(
	'wp_ajax_saga_request_summary',
	function () {
		// Security checks
		check_ajax_referer( 'saga_summaries_nonce', 'nonce' );

		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( array( 'message' => summary_i18n_text( 'error_unauthorized' ) ), 403 );
		}
		...
		if ( ! $rate_limiter->check( 'summary_generation', $user_id, 20, HOUR_IN_SECONDS ) ) {
			$reset_time = $rate_limiter->get_reset_time( 'summary_generation', $user_id, HOUR_IN_SECONDS );
			wp_send_json_error(
				array(
					'message' => sprintf(
						__( 'Rate limit exceeded. Maximum 20 summaries per hour. Try again in %d minutes.', 'saga-manager' ),
						ceil( $reset_time / 60 )
					),
				),
				429
		...
		$scope_params = isset( $_POST['scope_params'] ) ? (array) $_POST['scope_params'] : array();
		...
		// Validation
		if ( $saga_id === 0 ) {
			wp_send_json_error( array( 'message' => summary_i18n_text( 'error_no_saga' ) ), 400 );
		}

		if ( empty( $summary_type ) ) {
			wp_send_json_error( array( 'message' => summary_i18n_text( 'error_no_type' ) ), 400 );
		}

		try {
			$type = SummaryType::from( $summary_type );
		} catch ( \ValueError $e ) {
			wp_send_json_error( array( 'message' => __( 'Invalid summary type', 'saga-manager' ) ), 400 );
		}

		// Check entity requirement
		if ( $type->requiresEntity() && $entity_id === null ) {
			wp_send_json_error( array( 'message' => summary_i18n_text( 'error_no_entity' ) ), 400 );
		}
		...
		try {
			...
			$rate_limiter->record( 'summary_generation', $user_id, true );
 ...
 */
add_action(
	'wp_ajax_saga_get_summary_progress',
	function () {
		check_ajax_referer( 'saga_summaries_nonce', 'nonce' );
		...
		if ( $request_id === 0 ) {
			wp_send_json_error( array( 'message' => __( 'Invalid request ID', 'saga-manager' ) ), 400 );
		}
 ...
 */
add_action(
	'wp_ajax_saga_load_summary_detail',
	function () {
		check_ajax_referer( 'saga_summaries_nonce', 'nonce' );
		...
		if ( $summary_id === 0 ) {
			wp_send_json_error( array( 'message' => __( 'Invalid summary ID', 'saga-manager' ) ), 400 );
		}
		...
		try {
			...
			if ( ! $summary ) {
				wp_send_json_error( array( 'message' => summary_i18n_text( 'error_not_found' ) ), 404 );
			}
 ...
 */
add_action(
	'wp_ajax_saga_regenerate_summary',
	function () {
		check_ajax_referer( 'saga_summaries_nonce', 'nonce' );
...
add_action(
	...
	function () {
		...
		try {
			...
			$summary = $repository->findById( $summary_id );
			if ( ! $summary ) {
				wp_send_json_error( array( 'message' => __( 'Summary not found', 'saga-manager' ) ), 404 );
			}

			// Check if user has permission to delete this specific summary
			// Allow if user created it OR has delete_others_posts capability
			$current_user_id = get_current_user_id();
			if ( $summary->created_by !== $current_user_id && ! current_user_can( 'delete_others_posts' ) ) {
				wp_send_json_error( array( 'message' => __( 'You do not have permission to delete this summary', 'saga-manager' ) ), 403 );
			}
 ...
 */
add_action(
	'wp_ajax_saga_submit_summary_feedback',
	function () {
		check_ajax_referer( 'saga_summaries_nonce', 'nonce' );
		...
		if ( $rating < 1 || $rating > 5 ) {
			wp_send_json_error( array( 'message' => __( 'Rating must be between 1 and 5', 'saga-manager' ) ), 400 );
		}
 ...
 */
add_action(
	'wp_ajax_saga_get_summary_statistics',
	function () {
		check_ajax_referer( 'saga_summaries_nonce', 'nonce' );
...
add_action(
	...
	function () {
		...
		if ( empty( $search_term ) ) {
			wp_send_json_error( array( 'message' => __( 'Search term cannot be empty', 'saga-manager' ) ), 400 );
		}
