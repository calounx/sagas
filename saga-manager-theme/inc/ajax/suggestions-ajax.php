...
function saga_ajax_generate_suggestions(): void {
    if (!saga_verify_suggestions_ajax()) return;

    $saga_id = absint($_POST['saga_id'] ?? 0);
    if (!$saga_id) {
        wp_send_json_error(['message' => 'Invalid saga ID']);
        return;
    }
    ...
    try {
        ...
        if ($success) {
            ...
            wp_send_json_error(['message' => 'Failed to schedule job (may already be running)']);
        }
...
function saga_ajax_load_suggestions(): void {
    ...
    try {
        ...
        // Valid sort columns
        $valid_sorts = ['confidence_score', 'priority_score', 'created_at'];
        ...
        wp_send_json_error(['message' => 'Failed to load suggestions']);
    }
}
...
function saga_ajax_accept_suggestion(): void {
    ...
    if (!$suggestion_id) {
        wp_send_json_error(['message' => 'Invalid suggestion ID']);
        return;
    }
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Failed to accept suggestion']);
    }
}
...
function saga_ajax_reject_suggestion(): void {
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Failed to reject suggestion']);
    }
}
...
function saga_ajax_modify_suggestion(): void {
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Failed to modify suggestion']);
    }
}
...
function saga_ajax_dismiss_suggestion(): void {
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Failed to dismiss suggestion']);
    }
}
...
function saga_ajax_bulk_accept_suggestions(): void {
    ...
    $suggestion_ids = array_map('absint', $_POST['suggestion_ids'] ?? []);
    if (empty($suggestion_ids)) {
        wp_send_json_error(['message' => 'No suggestions provided']);
        return;
    }
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Bulk operation failed']);
    }
}
...
function saga_ajax_create_relationship_from_suggestion(): void {
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Failed to create relationship']);
    }
}
...
function saga_ajax_get_suggestion_details(): void {
    ...
    try {
        ...
        $source_entity = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$entities_table} WHERE id = %d",
            $suggestion->getSourceEntityId()
        ), ARRAY_A);

        $target_entity = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$entities_table} WHERE id = %d",
        ...
        wp_send_json_error(['message' => 'Failed to get suggestion details']);
    }
}
...
function saga_ajax_get_learning_stats(): void {
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Failed to get learning statistics']);
    }
}
...
function saga_ajax_get_suggestion_analytics(): void {
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Failed to get analytics']);
    }
}
...
function saga_ajax_trigger_learning_update(): void {
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Failed to update learning weights']);
    }
}
...
function saga_ajax_reset_learning(): void {
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Failed to reset learning weights']);
    }
}
