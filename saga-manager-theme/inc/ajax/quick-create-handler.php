...
class QuickCreateHandler
{
    ...
    {
        // Main create action
        add_action('wp_ajax_saga_quick_create', [$this, 'handle_create']);
        ...
        add_action('wp_ajax_saga_search_entities', [$this, 'handle_search_entities']);

        // Attribute definitions
        add_action('wp_ajax_saga_get_attributes', [$this, 'handle_get_attributes']);
    }
    ...
    public function handle_get_templates(): void
    {
        ...
        if (empty($entity_type)) {
            wp_send_json_error(['message' => 'Entity type required'], 400);
        }
    ...
    private function sanitize_input(array $raw_data): array
    {
        return [
            ...
            'status' => in_array($raw_data['status'] ?? 'draft', ['draft', 'publish'])
                ? $raw_data['status']
                : 'draft',
    ...
    private function update_quality_metrics(int $entity_id): void
    {
        ...
        $required_count = $wpdb->get_var($wpdb->prepare(
