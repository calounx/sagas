 ...
 */
function saga_filter_galaxy_data($nodes, $links, $filters = []) {
    // Filter by entity types
    if (!empty($filters['types'])) {
        $allowed_types = array_map('sanitize_key', $filters['types']);
        $nodes = array_filter($nodes, function($node) use ($allowed_types) {
        ...
        $links = array_filter($links, function($link) use ($node_ids) {
    ...
    if (!empty($filters['min_importance'])) {
        ...
        $nodes = array_filter($nodes, function($node) use ($min_importance) {
    ...
    if (!empty($filters['min_connections'])) {
        ...
        $nodes = array_filter($nodes, function($node) use ($min_connections) {
