...
final class Saga_Consistency_Ajax_Handler
{
    ...
    public function __construct()
    {
        ...
        // Register AJAX hooks
        add_action('wp_ajax_saga_run_consistency_scan', [$this, 'runConsistencyScan']);
        add_action('wp_ajax_saga_get_scan_progress', [$this, 'getScanProgress']);
        add_action('wp_ajax_saga_resolve_issue', [$this, 'resolveIssue']);
        add_action('wp_ajax_saga_dismiss_issue', [$this, 'dismissIssue']);
        add_action('wp_ajax_saga_bulk_action', [$this, 'bulkAction']);
        add_action('wp_ajax_saga_export_issues', [$this, 'exportIssues']);
        add_action('wp_ajax_saga_get_issue_details', [$this, 'getIssueDetails']);
        add_action('wp_ajax_saga_load_issues', [$this, 'loadIssues']);

        // Real-time editor endpoints
        add_action('wp_ajax_saga_check_entity_realtime', [$this, 'checkEntityRealtime']);
        add_action('wp_ajax_saga_get_entity_issues', [$this, 'getEntityIssues']);
        add_action('wp_ajax_saga_dismiss_inline_warning', [$this, 'dismissInlineWarning']);
    }
    ...
    public function runConsistencyScan(): void
    {
        ...
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => __('Permission denied', 'saga-manager-theme')], 403);
        }

        $sagaId = absint($_POST['saga_id'] ?? 0);

        if ($sagaId === 0) {
            wp_send_json_error(['message' => __('Invalid saga ID', 'saga-manager-theme')], 400);
        }

        // Initialize progress tracking
        $progressKey = self::SCAN_PROGRESS_KEY . $sagaId;
        set_transient($progressKey, [
            'status' => 'running',
            'progress' => 0,
    ...
    public function getScanProgress(): void
    {
        ...
        if ($progress === false) {
            wp_send_json_error(['message' => __('No active scan found', 'saga-manager-theme')], 404);
        }
    ...
    public function resolveIssue(): void
    {
        ...
        if ($issueId === 0) {
            wp_send_json_error(['message' => __('Invalid issue ID', 'saga-manager-theme')], 400);
        }
    ...
    public function bulkAction(): void
    {
        ...
        if (empty($issueIds)) {
            wp_send_json_error(['message' => __('No issues selected', 'saga-manager-theme')], 400);
        }
        ...
        wp_send_json_success([
            'message' => sprintf(
                __('Processed %d issues (%d successful, %d failed)', 'saga-manager-theme'),
                count($issueIds),
                $successCount,
                $failCount
            ),
    ...
    public function getIssueDetails(): void
    {
        ...
        try {
            ...
            if ($issue === null) {
                wp_send_json_error(['message' => __('Issue not found', 'saga-manager-theme')], 404);
            }
    ...
    public function loadIssues(): void
    {
        ...
        try {
            ...
            $issueData = array_map(function ($row) {
    ...
    public function checkEntityRealtime(): void
    {
        ...
        if ($entityId === 0) {
            wp_send_json_error(['message' => __('Invalid entity ID', 'saga-manager-theme')], 400);
        }
        ...
        try {
            ...
            if (!$entity) {
                wp_send_json_error(['message' => __('Entity not found', 'saga-manager-theme')], 404);
            }

            // Run lightweight check (only rule-based, no AI)
            $issues = $this->analyzer->analyze($entity->saga_id, [$entityId], false);
    ...
    public function dismissInlineWarning(): void
    {
        ...
        $dismissed = get_user_meta($userId, 'saga_dismissed_warnings', true) ?: [];
