...
final class ConsistencyRuleEngine {
	 ...
	 */
	public function runRules( int $sagaId, array $ruleTypes = array() ): array {
		$issues = array();

		if ( empty( $ruleTypes ) ) {
			$ruleTypes = array( 'timeline', 'character', 'location', 'relationship', 'logical' );
		}
	...
	private function checkTimelineRules( int $sagaId ): array {
		...
		$invalidDates = $this->wpdb->get_results(
			$this->wpdb->prepare(
		...
		$orphanedEvents = $this->wpdb->get_results(
			$this->wpdb->prepare(
		...
		foreach ( $orphanedEvents as $event ) {
			$issues[] = new ConsistencyIssue(
				...
				description: sprintf(
					__( 'Timeline event "%1$s" references non-existent entity ID %2$d', 'saga-manager-theme' ),
					$event->title,
	...
	private function checkCharacterRules( int $sagaId ): array {
		...
		$incompleteCharacters = $this->wpdb->get_results(
			$this->wpdb->prepare(
	...
	private function checkLocationRules( int $sagaId ): array {
		...
		$isolatedLocations = $this->wpdb->get_results(
			$this->wpdb->prepare(
				"SELECT e.id, e.canonical_name
            FROM {$this->prefix}entities e
            WHERE e.saga_id = %d
	...
	private function checkRelationshipRules( int $sagaId ): array {
		...
		$selfRefs = $this->wpdb->get_results(
			$this->wpdb->prepare(
		...
		foreach ( $selfRefs as $rel ) {
			$issues[] = new ConsistencyIssue(
				...
				relatedEntityId: null,
				description: sprintf(
					__( 'Entity "%1$s" has self-referencing %2$s relationship', 'saga-manager-theme' ),
					$rel->canonical_name,
		...
		$invalidTemporal = $this->wpdb->get_results(
			$this->wpdb->prepare(
		...
		foreach ( $invalidTemporal as $rel ) {
			$issues[] = new ConsistencyIssue(
				...
				relatedEntityId: null,
				description: sprintf(
					__( 'Relationship "%1$s → %2$s" (%3$s) has end date before start date', 'saga-manager-theme' ),
					$rel->source_name,
					$rel->target_name,
					$rel->relationship_type
				),
		...
		$invalidStrength = $this->wpdb->get_results(
			$this->wpdb->prepare(
				"SELECT r.id, e1.canonical_name as source_name, e2.canonical_name as target_name,
                   r.strength
            FROM {$this->prefix}entity_relationships r
		...
		foreach ( $invalidStrength as $rel ) {
			$issues[] = new ConsistencyIssue(
				...
				relatedEntityId: null,
				description: sprintf(
					__( 'Relationship "%1$s → %2$s" has invalid strength %3$d (must be 0-100)', 'saga-manager-theme' ),
					$rel->source_name,
					$rel->target_name,
					$rel->strength
				),
	...
	private function checkLogicalRules( int $sagaId ): array {
		...
		$invalidScores = $this->wpdb->get_results(
			$this->wpdb->prepare(
		...
		foreach ( $invalidScores as $entity ) {
			$issues[] = new ConsistencyIssue(
				...
				relatedEntityId: null,
				description: sprintf(
					__( 'Entity "%1$s" has invalid importance score %2$d (must be 0-100)', 'saga-manager-theme' ),
		...
		$duplicateSlugs = $this->wpdb->get_results(
			$this->wpdb->prepare(
		...
		foreach ( $duplicateSlugs as $duplicate ) {
			$issues[] = new ConsistencyIssue(
				...
				relatedEntityId: null,
				description: sprintf(
					__( 'Duplicate slug "%1$s" used by %2$d entities: %3$s', 'saga-manager-theme' ),
					$duplicate->slug,
					$duplicate->count,
					$duplicate->entities
				),
