...
use SagaManager\AI\Interfaces\SummaryRepositoryInterface;
 ...
 * Implements SummaryRepositoryInterface for Dependency Inversion.
 */
class SummaryRepository implements SummaryRepositoryInterface {
	 ...
	 * ]);
	 */
	public function findBySaga( int $saga_id, array $filters = array() ): array {
		global $wpdb;
	...
	public function getStatistics( int $saga_id ): array {
		...
		return array(
			...
			'by_type'           => $type_counts,
			'avg_quality_score' => $overall['avg_quality'] ? round( (float) $overall['avg_quality'], 2 ) : null,
	...
	public function createRequest( SummaryRequest $request ): int {
		...
		$data = array(
			...
			'created_at'       => $request->created_at ? date( 'Y-m-d H:i:s', $request->created_at ) : null,
		);
