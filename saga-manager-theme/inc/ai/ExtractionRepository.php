	...
	public function deleteJob( int $job_id ): bool {
		...
		try {
			...
			$wpdb->delete(
				...
				array( 'extracted_entity_id' => $wpdb->prepare( 'IN (SELECT id FROM %i WHERE job_id = %d)', $this->entities_table, $job_id ) ),
				array( '%d' )
			);

			// Delete extracted entities
			$wpdb->delete( $this->entities_table, array( 'job_id' => $job_id ), array( '%d' ) );

			// Delete job
			$result = $wpdb->delete( $this->jobs_table, array( 'id' => $job_id ), array( '%d' ) );
