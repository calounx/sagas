...
class SummaryGenerationService {
	...
	private function buildTemplateVariables( SummaryRequest $request, array $data ): array {
		...
		switch ( $request->summary_type ) {
			case SummaryType::CHARACTER_ARC:
				...
				$variables['timeline_events'] = $this->formatEvents( $data['events'] ?? array() );
				$variables['relationships']   = $this->formatRelationships( $data['relationships'] ?? array() );
				break;

			case SummaryType::TIMELINE:
				$variables['timeline_events']   = $formatted;
				$variables['entities']          = $this->formatEntities( $data['entities'] ?? array() );
			...
			case SummaryType::RELATIONSHIP:
				...
				$variables['entities']        = $this->formatEntities( $data['entities'] ?? array() );
				$variables['relevant_events'] = $this->formatEvents( $data['events'] ?? array() );
				break;

			case SummaryType::FACTION:
				$variables['faction_data'] = $formatted;
				$variables['members']      = $this->formatEntities( $data['members'] ?? array() );
				$variables['events']       = $this->formatEvents( $data['activities'] ?? array() );
				break;

			case SummaryType::LOCATION:
				$variables['location_data'] = $formatted;
				$variables['events']        = $this->formatEvents( $data['events'] ?? array() );
				$variables['entities']      = $this->formatEntities( $data['residents'] ?? array() );
				break;
		}
	...
	private function calculateQualityScore( string $text, array $data ): float {
		...
		// Check data coverage
		$source_count = count( $data['source_ids'] ?? array() );
		if ( $source_count > 0 ) {
