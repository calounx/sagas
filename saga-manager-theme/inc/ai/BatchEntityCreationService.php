...
class BatchEntityCreationService {
	...
	private function prepareAttributeValue( mixed $value ): array {
		if ( is_bool( $value ) ) {
			return array( 'value_bool' => $value ? 1 : 0 );
		}

		if ( is_int( $value ) ) {
			return array( 'value_int' => $value );
		}

		if ( is_float( $value ) ) {
			return array( 'value_float' => $value );
		}

		if ( is_array( $value ) ) {
			return array( 'value_json' => json_encode( $value ) );
		}
		...
		if ( mb_strlen( $str_value ) > 500 ) {
			return array( 'value_text' => $str_value );
		}

		return array( 'value_string' => $str_value );
	}
	...
	public function validateEntities( array $entities ): array {
		...
		foreach ( $entities as $entity ) {
			if ( ! ( $entity instanceof ExtractedEntity ) ) {
				$invalid[] = array(
					'entity' => null,
					'errors' => array( 'Not an ExtractedEntity object' ),
				);
				continue;
			}
