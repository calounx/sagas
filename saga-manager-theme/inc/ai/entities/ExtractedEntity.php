...
enum ExtractedEntityStatus: string
{
    ...
    {
        return in_array($this, [self::REJECTED, self::CREATED], true);
    }
}
...
final readonly class ExtractedEntity
{
    ...
    public static function fromArray(array $row): self
    {
        return new self(
            ...
            alternative_names: isset($row['alternative_names'])
                ? json_decode($row['alternative_names'], true) ?? []
                : [],
    ...
    {
        return array_unique(array_merge([$this->canonical_name], $this->alternative_names));
    }
    ...
    public function getQualityScore(): float
    {
        ...
        $completeness = 0.0;
        if (!empty($this->description)) $completeness += 40;
        if (!empty($this->alternative_names)) $completeness += 20;
        if (!empty($this->attributes)) $completeness += 20;
        if ($this->context_snippet !== null) $completeness += 20;
