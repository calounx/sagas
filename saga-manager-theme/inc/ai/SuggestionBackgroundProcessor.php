...
use SagaManager\AI\PredictiveRelationships\SuggestionRepository;
...
class SuggestionBackgroundProcessor {
	...
	public function init(): void {
		// Register cron hooks
		add_action( self::CRON_HOOK, array( $this, 'processSagaSuggestions' ), 10, 1 );
		add_action( self::DAILY_REFRESH_HOOK, array( $this, 'refreshAllSagas' ) );
	...
	public function scheduleGenerationJob( int $saga_id ): bool {
		...
		set_transient(
			"saga_suggestion_generation_{$saga_id}",
			array(
				'status'     => 'queued',
				'progress'   => 0,
				'total'      => 0,
		...
		wp_schedule_single_event( time(), self::CRON_HOOK, array( $saga_id ) );
