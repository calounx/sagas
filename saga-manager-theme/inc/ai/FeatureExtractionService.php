...
class FeatureExtractionService {
	...
	public function calculateCoOccurrence( int $entity1_id, int $entity2_id ): float {
		...
		// Fixed: Use fragment_text for co-occurrence instead of same ID (which always fails)
		$query = $wpdb->prepare(
	...
	public function getFeatureStatistics( int $saga_id ): array {
		...
		$stats['total_entities'] = (int) $wpdb->get_var(
			$wpdb->prepare(
		...
		$stats['total_relationships'] = (int) $wpdb->get_var(
			$wpdb->prepare(
		...
		$stats['total_timeline_events'] = (int) $wpdb->get_var(
			$wpdb->prepare(
	...
	/**
	 * Batch extract features for multiple entity pairs
	 *
	 * Optimizes feature extraction by using bulk queries instead of
	 * individual queries for each pair. Reduces N+1 query problems.
	 *
	 * @param array $entity_pairs Array of ['entity1_id' => int, 'entity2_id' => int, 'saga_id' => int]
	 * @return array Array indexed by "{entity1_id}_{entity2_id}" => features array
	 * @throws \Exception If extraction fails
	 */
	public function batchExtractFeatures( array $entity_pairs ): array {
		if ( empty( $entity_pairs ) ) {
			return array();
		}
		...
		foreach ( $entity_pairs as $pair ) {
			$entity_ids[] = $pair['entity1_id'];
			$entity_ids[] = $pair['entity2_id'];
		}
		...
		// Batch load content fragment counts
		$fragment_counts = $wpdb->get_results(
			$wpdb->prepare(
		...
		$fragment_count_map = array();
		foreach ( $fragment_counts as $row ) {
			$fragment_count_map[ $row['entity_id'] ] = (int) $row['count'];
		}

		// Batch load relationship counts
		$relationship_counts = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT
                source_entity_id,
                COUNT(*) as outgoing,
                (SELECT COUNT(*) FROM {$this->relationships_table} WHERE target_entity_id = r.source_entity_id) as incoming
		...
		$relationship_count_map = array();
		foreach ( $relationship_counts as $row ) {
			$relationship_count_map[ $row['source_entity_id'] ] = array(
		...
		foreach ( $entity_pairs as $pair ) {
			...
			$saga_id    = $pair['saga_id'];

			$key = "{$entity1_id}_{$entity2_id}";

			try {
				// Use cached data where possible, fall back to individual queries
				$features = array();

				// Co-occurrence - still needs individual query for pairs
				$features[ FeatureType::CO_OCCURRENCE->value ] = $this->calculateCoOccurrence(
					$entity1_id,
					$entity2_id
				);
				...
				// Attribute similarity - use cached entity data
				if ( isset( $entity_map[ $entity1_id ] ) && isset( $entity_map[ $entity2_id ] ) ) {
					$features[ FeatureType::ATTRIBUTE_SIMILARITY->value ] = $this->calculateAttributeSimilarityFromCache(
						$entity_map[ $entity1_id ],
						$entity_map[ $entity2_id ]
					);
				} else {
					$features[ FeatureType::ATTRIBUTE_SIMILARITY->value ] = 0.0;
				}
				...
				// Network centrality - use cached relationship counts
				if ( isset( $relationship_count_map[ $entity1_id ] ) && isset( $relationship_count_map[ $entity2_id ] ) ) {
					$total_entities = count( $entity_map );
					$degree1        = $relationship_count_map[ $entity1_id ]['outgoing'] + $relationship_count_map[ $entity1_id ]['incoming'];
					$degree2        = $relationship_count_map[ $entity2_id ]['outgoing'] + $relationship_count_map[ $entity2_id ]['incoming'];

					$centrality1                                        = $this->normalizeFeature( $degree1, 0, $total_entities );
					$centrality2                                        = $this->normalizeFeature( $degree2, 0, $total_entities );
					$features[ FeatureType::NETWORK_CENTRALITY->value ] = ( $centrality1 + $centrality2 ) / 2;
				} else {
					$features[ FeatureType::NETWORK_CENTRALITY->value ] = 0.0;
				}
				...
				error_log(
					sprintf(
						'[SAGA][PREDICTIVE][ERROR] Batch feature extraction failed for %d-%d: %s',
						$entity1_id,
						$entity2_id,
						$e->getMessage()
					)
	...
	/**
	 * Calculate attribute similarity from cached entity data
	 *
	 * @param array $entity1 Entity 1 data array
	 * @param array $entity2 Entity 2 data array
	 * @return float Normalized value 0-1
	 */
	private function calculateAttributeSimilarityFromCache( array $entity1, array $entity2 ): float {
		$similarity         = 0.0;
		$attributes_checked = 0;
