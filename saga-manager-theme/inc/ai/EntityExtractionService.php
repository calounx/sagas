...
class EntityExtractionService {
	 ...
	 */
	public function extractEntities( string $text, int $job_id, array $options = array() ): array {
	...
	private function extractFromChunk(
		...
		// Call AI API
		$response = $this->ai_client->complete(
			$prompt,
			array(
	...
	private function buildExtractionPrompt( string $text, array $options ): string {
		$entity_types = implode( ', ', array( 'character', 'location', 'event', 'faction', 'artifact', 'concept' ) );
		...
		foreach ( $entities_data as $data ) {
			try {
				...
				$entity = new ExtractedEntity(
					...
					alternative_names: $data['alternative_names'] ?? array(),
