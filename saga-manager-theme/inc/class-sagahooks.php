...
class SagaHooks {
	...
	public function registerHooks(): void {
		...
		add_filter( 'generate_sidebar_layout', array( $this, 'customizeSidebar' ) );
		add_filter( 'generate_content_width', array( $this, 'customizeContentWidth' ) );

		// Entity meta display
		add_action( 'generate_after_entry_title', array( $this, 'addEntityMeta' ) );
		add_action( 'generate_after_entry_content', array( $this, 'addEntityRelationships' ) );
		add_action( 'generate_after_entry_content', array( $this, 'addEntityAttributes' ) );

		// Archive customization
		add_action( 'generate_before_main_content', array( $this, 'addArchiveHeader' ) );
		add_filter( 'excerpt_length', array( $this, 'customizeExcerptLength' ), 999 );

		// Body classes
		add_filter( 'body_class', array( $this, 'addBodyClasses' ) );

		// Cache invalidation on post update
		add_action( 'save_post', array( $this, 'invalidateCacheOnSave' ), 10, 2 );
	}
