...
class QuickCreate {
	...
	public function init(): void {
		...
		// AJAX handlers
		add_action( 'wp_ajax_saga_quick_create', array( $this, 'handle_ajax_create' ) );
	...
	private function add_recent_entities( \WP_Admin_Bar $admin_bar ): void {
		...
		$admin_bar->add_menu(
			array(
				...
				'meta'   => array( 'class' => 'saga-recent-header' ),
			)
		);
	...
	private function sanitize_entity_data( array $raw_data ): array {
		return array(
			...
			'attributes'     => $raw_data['attributes'] ?? array(),
			'relationships'  => array_map( 'absint', $raw_data['relationships'] ?? array() ),
		);
	}
