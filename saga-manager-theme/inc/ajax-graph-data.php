...
function build_graph_data(
    ...
    if (!empty($nodes)) {
        ...
        $rel_where = ["(source_entity_id IN ({$placeholders}) OR target_entity_id IN ({$placeholders}))"];
...
function traverse_relationships(
    ...
    while (!empty($queue) && count($result) < $limit) {
        ...
        if ($current_depth < $max_depth) {
            ...
            $rel_where = ['(source_entity_id = %d OR target_entity_id = %d)'];
            $rel_args = [$current_id, $current_id];
            ...
            foreach ($relationships as $rel) {
                ...
                if (!isset($visited[$next_id])) {
                    $queue[] = ['id' => $next_id, 'depth' => $current_depth + 1];
                }
            }
        }
    }
...
function format_edge(array $rel_data): array {
    $metadata = !empty($rel_data['metadata']) ? json_decode($rel_data['metadata'], true) : [];
