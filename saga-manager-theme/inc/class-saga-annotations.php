...
class Saga_Annotations {
    ...
    public static function init(): void {
        ...
        add_action('wp_ajax_saga_save_annotation', [__CLASS__, 'ajax_save_annotation']);
        add_action('wp_ajax_saga_delete_annotation', [__CLASS__, 'ajax_delete_annotation']);
        add_action('wp_ajax_saga_search_annotations', [__CLASS__, 'ajax_search_annotations']);
        add_action('wp_ajax_saga_export_annotations', [__CLASS__, 'ajax_export_annotations']);
        add_action('wp_ajax_saga_get_annotations', [__CLASS__, 'ajax_get_annotations']);
        add_action('wp_ajax_saga_get_user_tags', [__CLASS__, 'ajax_get_user_tags']);
    }
    ...
    public static function enqueue_assets(): void {
        ...
        wp_localize_script('saga-annotations', 'sagaAnnotations', [
    ...
    public static function get_entity_annotations(int $entity_id, int $user_id = 0): array {
        ...
        return array_filter($all_annotations, function($annotation) use ($entity_id) {
    ...
    public static function search_annotations(string $query, int $user_id = 0): array {
        ...
        return array_filter($annotations, function($annotation) use ($query) {
            $searchable = strtolower(
                ($annotation['content'] ?? '') . ' ' .
                ($annotation['quote'] ?? '') . ' ' .
                implode(' ', $annotation['tags'] ?? [])
            );
    ...
    public static function ajax_save_annotation(): void {
        ...
        if (!is_user_logged_in()) {
            wp_send_json_error(['message' => __('You must be logged in.', 'saga-manager-theme')], 401);
        }
    ...
    public static function ajax_delete_annotation(): void {
        ...
        if (empty($annotation_id)) {
            wp_send_json_error(['message' => __('Invalid annotation ID.', 'saga-manager-theme')], 400);
        }
        ...
        wp_send_json_success(['message' => __('Annotation deleted successfully.', 'saga-manager-theme')]);
    }
    ...
    public static function ajax_search_annotations(): void {
        ...
        wp_send_json_success(['annotations' => array_values($annotations)]);
    }
