...
if [[ $WP_VERSION =~ ^[0-9]+\.[0-9]+\-(beta|RC) ]]; then
...
install_db() {
    ...
    # create database
    if [ $(mysql --user="$DB_USER" --password="$DB_PASS"$EXTRA --execute="SELECT 1" 2>/dev/null) ]; then
        mysql --user="$DB_USER" --password="$DB_PASS"$EXTRA --execute="CREATE DATABASE IF NOT EXISTS $DB_NAME"
        echo "Database $DB_NAME created or already exists"
    else
        echo "Could not connect to database. Please check credentials."
        exit 1
    fi
}
