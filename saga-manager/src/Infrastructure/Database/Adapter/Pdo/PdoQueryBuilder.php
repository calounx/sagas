 ...
 * PDO Query Builder
 *
 * Fluent interface for building SQL queries with PDO.
 *
 * @example
 *   $result = $qb
...
final class PdoQueryBuilder implements QueryBuilderInterface
{
    ...
    {
        if (empty($values)) {
            return $this; // No condition needed for empty NOT IN
        }
    ...
    {
        $sql = $this->toSql();
        $stmt = $this->connection->execute($sql, $this->parameters);
    ...
    public function toSql(): string
    {
        ...
        if (!empty($this->havings)) {
            ...
            foreach ($this->havings as $having) {
                $param = $this->addParameter($having['value']);
                $havingParts[] = "{$having['column']} {$having['operator']} {$param}";
            }
    ...
    private function buildInCondition(string $column, string $operator, array $values): string
    {
        ...
        foreach ($values as $value) {
            $placeholders[] = $this->addParameter($value);
        }
